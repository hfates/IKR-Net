...
def SwinSRnet(args, opt_P, opt_C):
    ...
    model.load_state_dict(torch.load('../experiments/train_8it_bld_ker/models/F_65000.pth'), strict=False)
    model.load_state_dict(torch.load('../checkpoints/usrnet.pth'), strict=False)
    
    model.h = DataParallel(model.h)
    model.swin = DataParallel(model.swin)
    
    model.load_state_dict(torch.load('../experiments/train_swin/models/F_384000.pth'), strict=False)
