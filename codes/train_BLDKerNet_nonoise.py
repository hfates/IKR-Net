...
#grad_cond = 'netP'
grad_cond1 = 'netP' ; grad_cond2 = 'dec.'
#grad_cond1 = 'p_init' ; grad_cond2 = 'h_init'
gr_cond_range = 4
grad_cond =  'pk'
#grad_cond1 = 'p.' ; grad_cond2 = 'h.'
...
def BLDSRnet(opt_P, opt_C):
    ...
    # #pretrained_dict_init = {k[0]+"_init"+k[1:]: v for (k, v) in pretrained_dict.items() if ((k[0]=='p') or (k[0]=='h'))}
    # pretrained_dict_init = {k[0:2]+"module."+k[2:]: v for (k, v) in pretrained_dict.items() if ((k[0]=='p') or (k[0]=='h'))}
    ...
    for key, v in model.named_parameters():
        #print(key)
        #if (key[0:gr_cond_range]==grad_cond):
        if (key[0:gr_cond_range]==grad_cond1) | (key[0:gr_cond_range]==grad_cond2):
        #if (key[0:4]==grad_cond1) | (key[0:4]==grad_cond2):
...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            SR_img, ker_img, b_check, est_ker_map, est_noise = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, train_data['GT'])             
            ...
            opt_dict = {'sr_p': cropped_sr_img, 'sr_t':cropped_gt_img, 'code_p': est_ker_map, 'code_t': kernel_code, 'ker_p':ker_img, 'ker_t':gt_ker, 'nois_p':est_noise, 'nois_t':,}
            ...
            if current_step % opt_P['train']['val_freq'] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    for filt_ind in sel_ker:        
                        ...
                        sigma = torch.tensor([val_noise]*B).float().view([B, 1, 1, 1]).to(device)
                        SR_it = []
                        ker_it = []
