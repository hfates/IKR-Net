...
from models.BLDSRnet_v2 import BLDNoiseNet as net
...
def BLDSRnet(opt_P, opt_C):
    ...
    #model.load_state_dict(torch.load('../checkpoints/usrnet.pth'), strict=False)
    #model.load_state_dict(torch.load('../experiments/train_bldsr_init/models/F_35000.pth'), strict=True)
    ...
    model.load_state_dict(torch.load('../experiments/Noise_pred/models/F_1000.pth'), strict=False)
...
def main():
    ...
    prepro = util.USRNetTestMotionVarNoise(opt_P['scale'], pca_matrix, random=True  , para_input=opt_P['code_length'],
                                              kernel=opt_P['kernel_size'], noise=True, cuda=True, sig=opt_P['sig'],
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            SR_img, ker_img, b_check, est_noise = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, train_data['GT'])             
            ...
            fl1=F1_loss(est_noise,(sigma.view(B))/noise_level)
            fl = fl1
            fl.backward()
            F_opt.step()
            log_loss += fl.item()
            ...
            if current_step % opt_P['train']['val_freq'] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    prepro_val = util.USRNetTestVarNoise(opt_P['scale'], pca_matrix, random=True  , para_input=opt_P['code_length'],
                                                              kernel=opt_P['kernel_size'], noise=True, cuda=True, sig=opt_P['sig'],
                                                              sig_min=opt_P['sig_min'], sig_max=opt_P['sig_max'], rate_iso=1.0, scaling=3,
                                                              rate_cln=0.2, noise_high=noise_level)
                    ...
                    #sigma = torch.tensor([noise_level_rand]*B).float().view([B, 1, 1, 1]).to(device)
                    sigma = torch.tensor(noise_level_rand).float().view([B, 1, 1, 1]).to(device)
                    SR_it = []
                    ker_it = []
                    ...
                    with torch.no_grad():
                        SR_img, ker_est, b_check, est_noise = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, val_data['GT'], SR_it, ker_it) 
                    ...
                    avg_mse  += util.calculate_mse (est_noise.detach().cpu().numpy() , noise_level_rand/noise_level )
