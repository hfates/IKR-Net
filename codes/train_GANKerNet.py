...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            model_SR.train()
            model_GAN.update_learning_rate(current_step)
            ...
            #SR_img, ker_img, b_check = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, train_data['GT'])             
            
            model_GAN.feed_data(LR_img, train_data['GT'], b_kernels, opt_P['scale'], sigma)
            ...
            # F_opt.step()
            # log_loss += fl.item()
            model_GAN.optimize_parameters(current_step)

            #model_SR.ker.log_dict['F_loss'] = log_loss / loss_step
            model_SR.ker.log_dict['F_loss'] = model_GAN.log_dict['G_loss']
            ...
            if current_step % opt_P['logger']['save_checkpoint_freq'] == 0:
                if rank <= 0:
                    ...
                    torch.save(model_GAN.netG.state_dict(), opt_P['path']['log'] + r'/models/G_'+str(current_step)+'.pth')
                    model_SR.pre.save_training_state(epoch, current_step)
                    torch.save(model_GAN.netD.state_dict(), opt_P['path']['log'] + r'/models/D_'+str(current_step)+'.pth')
