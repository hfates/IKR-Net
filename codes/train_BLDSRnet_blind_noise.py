...
def BLDSRnet(opt_P, opt_C):
    ...
    #model.load_state_dict(torch.load('../experiments/full_usr_BSD68_ALL/save_models/USRNet_fine_tuned/F_160000.pth'), strict=True)
    ...
    model.load_state_dict(torch.load('../experiments/full_usr_ALL_noise01/full_models/F_57000.pth'), strict=True)
...
def main():
    ...
    motion_kernels=torch.load('encoder/motion.m')
    gaussian=torch.from_numpy(torch.load('encoder/ai.m'))
    motion_fixed=torch.from_numpy(torch.load('encoder/m.m'))
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt_P['logger']['save_checkpoint_freq'] == 0:
                if rank <= 0:
                    ...
                    torch.save(model_SR.state_dict(), r'../experiments/full_usr_ALL_noise01/F_'+str(current_step)+'.pth')
