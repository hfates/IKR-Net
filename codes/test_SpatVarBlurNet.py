...
from spat_varying import validation_SV 
from spat_varying.generate_blur_kernels_SV import BatchSRKernel_SV
from spat_varying.apply_blurring_SV import BatchBlur_SV
import time
...
def BLDSRnet(opt_P, opt_C):
    ...
    #model.load_state_dict(torch.load('../experiments/train_USRit_bld_ker_USRfine/models/F_67000.pth'), strict=True)
    model.load_state_dict(torch.load('../experiments/blur_sc1_Pre/models_final_wrong/F_88000.pth'), strict=True)
...
def main():
    ...
    parser.add_argument('-opt_P', type=str, default="options/train/USRNet_exp.yml", help='Path to option YMAL file of Predictor.')
    parser.add_argument('-opt_C', type=str, default="options/train/USRNet_exp_c.yml", help='Path to option YMAL file of Corrector.')
    ...
    pca = validation_SV.Predictor().to(device)   
    model_path = './spat_varying/sv/model_epoch_93_loss_0.007353409838076914_pth'
    pca.load_state_dict(torch.load(model_path), strict=True) 
    BlurSV = BatchBlur_SV(l=21, padmode='replication')
    ...
    crop_size = max(opt_P['scale']**2, 10)
        ...
        for filt_ind in [0]:
            ...
            if (1):
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    B = 100
                    sv_mode = 4
                    SVKernel_gen = BatchSRKernel_SV(l=21, sig=2.6, sig1=2.6, sig2=2.6, theta=0, sig_min=0.2, sig_max=4.0, rate_iso=.0, scale=1, divide_H=32, divide_W=32, sv_mode=sv_mode)
                    kernel_map, params = SVKernel_gen(random=False, batch=1, img_H=320, img_W=320, tensor=True)
                    kernel_map_full = kernel_map.view(1, 320, 320, 21, 21)
                    kernel_map_grid = kernel_map_full[0, 0::32, 0::32, ...]
                    
                    #kernel_map[:,:,:,:] = kernel_map[:,0,:,:]
                    
                    kernel = kernel_map_grid.reshape(-1, 21 * 21).to(device)
                    or_encoded = model_SR.enc(kernel) # [b, 10]
                    or_decoded = model_SR.dec(or_encoded) # [b, 441]
                    
                    degraded, padOut = BlurSV(val_data['GT'][:,:,:320,:320].to(device), kernel_map.to(device))
                    
                    start_time = time.time()
                    encoded = torch.transpose(pca(degraded), 1, 2).squeeze(0) # [100, 10]
                    reconstruction = model_SR.dec(encoded) # [100, 441]
                    tot_time += time.time() - start_time
                    
                    mse = F2_loss(or_decoded, reconstruction)
                    print('Image {}, MSE: {:.6f}'.format(idx, mse))
                                       
                    # display reconstruction
                    '''
                    reconstruction = reconstruction.detach().cpu()
                    or_kernel = kernel.detach().cpu()                                     
                    plt.figure
                    ax = plt.subplot(1, 1, 1)
                    plt.imshow(val_data['GT'][:,:,:320,:320].squeeze(0).detach().cpu().numpy().transpose(1,2,0))
                    # plt.gray()
                    plt.title('crop')
                    ax.get_xaxis().set_visible(False)
                    ax.get_yaxis().set_visible(False)
                    
                    reconstruction = reconstruction.view(10, 10, 21, 21)
                    or_kernel_plot = or_kernel.view(10, 10, 21, 21)
                    ...
                    for j in range(10):
                        for k in range(10):
                            ...
                            #plt.imshow(reconstruction[j][k])
                            plt.imshow(or_kernel_plot[j][k])
                            ax.get_xaxis().set_visible(False)
                            ax.get_yaxis().set_visible(False)
                            count += 1
                    plt.show()
                    ...
                    start_time = time.time()
                    with torch.no_grad():
                        #SR_img, ker_est, b_check, _ = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, val_data['GT'], SR_it, ker_it) 
                        #SR_img, ker_est, b_check, _ = model_SR(padOut, b_kernels, opt_P['scale'], sigma, val_data['GT'], SR_it, ker_it) 
                        SR_img, ker_est, b_check, _ = model_SR(padOut, reconstruction.view(100,21,21), opt_P['scale'], sigma, val_data['GT'], SR_it, ker_it) 

                        pad2UDC = SR_img.permute(1, 0, 2,3)[:,:,16:48,16:48].reshape(3,100,32*32).permute(0,2,1)
                        SR_img = torch.nn.functional.fold(pad2UDC, (320, 320), stride=32, kernel_size=32).view(1, 3, 320, 320)
                    tot_time += time.time() - start_time
                    
                    sigma_2 = torch.tensor([noise_level]*1).float().view([1, 1, 1, 1]).to(device)
                    start_time = time.time()
                    with torch.no_grad():
                        #SR_img, ker_est, b_check, _ = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, val_data['GT'], SR_it, ker_it) 
                        SR_img_2, ker_est_2, b_check_2, _ = model_SR(degraded, b_kernels, opt_P['scale'], sigma_2, val_data['GT'], SR_it, ker_it) 
                    ...
                    gt_img = util.tensor2img(val_data['GT'][:,:,:320,:320].detach().cpu())  # uint8
                    sr_img = util.tensor2img(SR_img.detach().cpu())  # uint8
                    sr_img_2 = util.tensor2img(SR_img_2.detach().cpu())  # uint8
                    ...
                    util.save_img(sr_img, save_img_path)
                    save_img_path = os.path.join(img_dir, '{:s}_{:d}_2.png'.format(img_name, 8))
                    util.save_img(sr_img_2, save_img_path)
                    ...
                    gt_img = gt_img / 255.
                    sr_img = sr_img / 255.
                    sr_img_2 = sr_img_2 / 255.
                    cropped_sr_img = sr_img[crop_size:-crop_size, crop_size:-crop_size, :]
                    cropped_sr_img_2 = sr_img_2[crop_size:-crop_size, crop_size:-crop_size, :]
                    cropped_gt_img = gt_img[crop_size:-crop_size, crop_size:-crop_size, :]
                    step_psnr = util.calculate_psnr(cropped_sr_img * 255, cropped_gt_img * 255)
                    step_ssim = util.calculate_psnr(cropped_sr_img_2 * 255, cropped_gt_img * 255)
                ...
                logger_val.info('<epoch:{:3d}, iter:{:8,d}, step:{:3d}> img psnr: {:.6f}, img_2 psnr: {:.6f},'.format(epoch, current_step, step, img_psnr, img_ssim))
                logger_val.info('<epoch:{:3d}, iter:{:8,d}, step:{:3d}> time: {:.6f}, time_2: {:.6f},'.format(epoch, current_step, step,  tot_time/idx, tot_time_2/idx))
                # tensorboard logger
                if opt_P['use_tb_logger'] and 'debug' not in opt_P['name']:
