...
from models.BLDSR_SCU import BLDKerNet as net
#from models.SCU_IKRNet import BLDKerNet as net
...
#model_file = '../experiments/USR8it_bldker_noise_est/models/F_9500.pth'
model_file = '../experiments/USR8it_bldker_noise/models/models_p2/F_46000.pth'
#model_file = '../experiments/Swin_bldker_noise_est_sc4/swin+usrnet/models/F_25000.pth'
#model_file = '../experiments/Swin_bldker_noise_est_sc4/models_swin_last/F_10000.pth'
#model_file = '../checkpoints/usrnet.pth'
#model_file = '../experiments/Swin_bldker_noise_est_sc4/models_SCU8it/F_35000.pth'
#model_file = '../experiments/Swin_bldker_noise_est_sc4/models_scu4it_blind/F_70000.pth'
model_file = '../experiments/Swin_bldker_noise_est_sc4/models/F_12500.pth'
...
def SCUSRnet(args,opt_P, opt_C):
    ...
    #model.load_state_dict(torch.load('../experiments/Noise_pred/models/F_5000.pth'), strict=False)
    ...
    pretrained_dict = torch.load('../experiments/USR8it_bldker_noise_est/models/F_9750.pth')
    ...
    pretrained_dict_init = {k : v for (k, v) in pretrained_dict.items() if ((k[0:4]=='nois'))}
    model.load_state_dict(pretrained_dict_init, strict=False)
    
  
    # pretrained_dict = torch.load('../checkpoints/usrnet.pth')
    # pretrained_dict_swin = {k[0]+"_swin"+k[1:]: v for (k, v) in pretrained_dict.items() if ((k[0:2]=='h.'))}
    # model.load_state_dict(pretrained_dict_swin, strict=False)
...
def main():
    ...
    parser.add_argument('-opt_P', type=str, default="options/train/train_BLDSR_noise_P.yml", help='Path to option YMAL file of Predictor.')
    parser.add_argument('-opt_C', type=str, default="options/train/train_BLDSR_noise_C.yml", help='Path to option YMAL file of Corrector.')
    ...
    parser.add_argument('--model_zoo', type=str, default='model_zoo/scunet', help='path of model_zoo')
    ...
    model_scu_path = os.path.join(args.model_zoo, args.model_name+'.pth')
    ...
    #### create model
    model_SR = SCUSRnet(args,opt_P,opt_C)  #load pretrained model of USRNET
    #model_SR.swin.load_state_dict(torch.load(model_scu_path), strict=True)
    ...
    prepro = util.USRNetTestMotionVarNoise(opt_P['scale'], pca_matrix, random=True  , para_input=opt_P['code_length'],
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            #     #                                           sig_min=opt_P['sig_min'], sig_max=opt_P['sig_max'], rate_iso=1.0, scaling=3,
            #     #                                           rate_cln=0.2, noise_high=0.0)
            LR_img, b_kernels, noise_level_rand = prepro(train_data['GT'], b_kernels, kernel=True)
            ...
            SR_img, ker_img, b_check, est_ker_map, est_noise = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, noise_level, train_data['GT'])             
            ...
            norm_noise = (sigma.view(B))/noise_level
            opt_dict = {'sr_p': cropped_sr_img, 'sr_t':cropped_gt_img, 'code_p': est_ker_map, 'code_t': kernel_code, 'ker_p':ker_img, 'ker_t':gt_ker, 'nois_p': est_noise, 'nois_t': norm_noise}
            ...
            if current_step % opt_P['train']['val_freq'] == 0 and rank <= 0:
                ...
                idx = 0
                model_SR.eval()
                for val_im_no, val_data in enumerate(val_loader):
                    if val_im_no == 50:
                        time.sleep(60)
                    # if (0):
                    # #if (np.random.randint(2)):
                    ...
                    for filt_ind in sel_ker:        
                        ...
                        with torch.no_grad():
                            SR_img, ker_est, b_check, est_ker_map, est_noise  = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, noise_level, val_data['GT'],  SR_it, ker_it) 
                        ...
                        avg_mse  += util.calculate_mse (est_noise.detach().cpu().numpy() , noise_level_rand/val_noise )
