...
from models.BLDSRnet_v3_rev import BLDKerNet as net
...
def BLDSRnet(opt_P, opt_C):
    ...
    # #pretrained_dict_init = {k[0]+k[6:] : v for (k, v) in pretrained_dict.items() if ((k[0:7]=='p_init.') or (k[0:7]=='h_init.'))}
    # pretrained_dict_init = {k[0]+"_init"+k[1:]: v for (k, v) in pretrained_dict.items() if ((k[0:2]=='p.') or (k[0:2]=='h.'))}
    # model.load_state_dict(pretrained_dict_init, strict=False)
        ...
        #for filt_ind in [11]:
            ...
            if (1):
                ...
                for im_no, val_data in enumerate(val_loader):
                    #if (idx>10):
                    #    break
                    # if not('147091' in val_data['LQ_path'][0]):
                    #     continue
                    # if (0):
                    ...
                    with torch.no_grad():
                        SR_img, ker_est, b_check, est_ker_map, est_noise = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, max_noise, val_data['GT'], SR_it, ker_it, os.path.join(img_dir, img_name)) 
