...
#blur = util.BatchBlur(l=kernel.shape[0])
...
for test_loader in test_loaders:
    ...
    for test_data in test_loader:
        ...
        #LR_img = ndimage.filters.convolve(test_data['GT'][0], filt[..., np.newaxis], mode='wrap')  # blur
        #LR_img2 = ndimage.filters.convolve(test_data['GT'][0].permute(1,2,0), filt[..., np.newaxis], mode='wrap')  # blur
        #LR_img2=LR_img2[::opt_P['scale'], ::opt_P['scale'], ...]
         # downsample, standard s-fold downsampler
        #LR_img2=torch.from_numpy(np.ascontiguousarray(LR_img2)).permute(2, 0, 1).float().unsqueeze(0).to(device)
