...
#model_file = '../experiments/USR8it_bldker_noise_est/models/F_4000.pth'
#model_file = '../experiments/USR8it_bldker_noise_sc2/models/models_p2/F_10000.pth'
#model_file = '../experiments/Swin_bldker_noise_est_sc4/swin_last_iter/models/F_35000.pth'
...
#model_file = '../experiments/Swin_bldker_noise_est_sc4/models_swin_usrnet_ft/F_20000.pth'
model_file = '../experiments/Swin_bldker_noise_est_sc4/models/F_15000.pth'
...
def SCUSRnet(args,opt_P, opt_C, model_scu_path):
    ...
    # mfile = '../experiments/USR8it_bldker_noise/models/models_p2/F_46000.pth'
    ...
    # pretrained_dict = torch.load('../experiments/Swin_bldker_noise_est_sc4/swin_oracle_ker/models/F_20000.pth')
    # pretrained_dict_init = {k[0:5]+"module."+k[5:]: v for (k, v) in pretrained_dict.items() if ((k[0:5]=='swin.'))}
    # model.load_state_dict(pretrained_dict_init, strict=False)
    # pretrained_dict_init = {k[0:7]+"module."+k[7:]: v for (k, v) in pretrained_dict.items() if ((k[0:7]=='h_swin.'))}
    # model.load_state_dict(pretrained_dict_init, strict=False)
    # pretrained_dict = torch.load('../checkpoints/usrnet.pth')
    # pretrained_dict_init = {k[0:2]+"module."+k[2:]: v for (k, v) in pretrained_dict.items() if ((k[0:2]=='p.') or (k[0:2]=='h.'))}
    # model.load_state_dict(pretrained_dict_init, strict=False)
    
    #model.swin.load_state_dict(torch.load(model_scu_path), strict=True)
    # pretrained_dict = torch.load(model_scu_path)
    # pretrained_dict_init = {k[5:] : v for (k, v) in pretrained_dict.items() if ((k[0:4]=='swin'))}
    # model.swin.load_state_dict(pretrained_dict_init, strict=False)
...
def main():
    ...
    parser.add_argument('-opt_P', type=str, default="options/test/test_BLDSR_noise_P.yml", help='Path to option YMAL file of Predictor.')
    parser.add_argument('-opt_C', type=str, default="options/test/test_BLDSR_noise_C.yml", help='Path to option YMAL file of Corrector.')
    ...
    model_scu_path = os.path.join(args.model_zoo, "F_6000"+'.pth')
    ...
    model_SR = SCUSRnet(args,opt_P,opt_C, model_scu_path)  #load pretrained model of USRNET
    ...
    for epoch in range(1):
        ...
        #for filt_ind in [11]:
            ...
            if (1):
                ...
                idx = 0
                model_SR.eval()
                for im_no, val_data in enumerate(val_loader):
                    #time.sleep(2)
                    #print(im_no)
                    #if (idx>10):
                    #    break
                    ...
                    with torch.no_grad():
                        SR_img, ker_est, b_check, est_ker_map, est_noise = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, max_noise, val_data['GT'], SR_it, ker_it) 
                    ...
                    avg_mse  += util.calculate_mse (est_noise.detach().cpu().numpy() , noise_level_rand/max_noise )
