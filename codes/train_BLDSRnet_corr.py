...
def define_optimizer(model):
    ...
    #G_optimizer = torch.optim.SGD(G_optim_params, lr=1e-4, momentum=0.9)
    return G_optimizer
...
def main():
    ...
    # X = ker_Arr.view(filt_no,441).cpu().numpy()
    # mu = np.mean(X, axis=0)
    ...
    # Xhat += mu
    # ker_Arr_PCA = torch.from_numpy(Xhat).view(filt_no,21,21)
    
    logger.info('Done filter generation')
    #### training  
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        while (ind_it<filt_no):
            ...
            #     b_kernels = util.fixed_batch_kernel(filt,opt_C['datasets']['train']['batch_size'])
            b_kernels = ker_Arr[idxAll[ind_it:ind_it+ba_size],:,:]
            ind_it += ba_size
            #kernel_code = enc(b_kernels) 
            B, H, W = b_kernels.size() #[B, l, l]
    
            # noise = torch.mul(torch.FloatTensor(np.random.normal(loc=0, scale=1.0, size=b_kernels.size())), 0.001).to(device)
            ...
            #ker_est, est_ker_map = model_SR.netC(LR_img, train_data['GT'].to(device))
            ker_est = model_SR.netC(b_kernels.view(B,1,H,W).detach())             
            # #update USRNet weights
            # # torch.autograd.set_detect_anomaly(True)
            #fl=F_loss(SR_img,train_data['GT'].to(device))
            fl=F_loss(ker_est,b_kernels.view(B,1,H,W))
            ...
            if current_step % opt_P['train']['val_freq'] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    if (0):
                        ...
                        b_kernels = util.fixed_batch_kernel(filt,1)
                    ...
                    with torch.no_grad():
                        ...
                        #ker_est, _ = model_SR.netC(LR_img, val_data['GT'].to(device))  
                        ker_est = model_SR.netC(b_kernels.view(B,1,H,W).to(device)) 
