...
def main():
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            #HFA
            # Initialise Kernel
            # kernel = Kernel(size=(21, 21), intensity=0.4)
            # # Display kernel
            ...
            fl=F_loss(ker_img,b_kernels.view(B,1,H,W).to(device))
            fl.backward()
            F_opt.step()
            ...
            if current_step % opt_P['train']['val_freq'] == 0 and rank <= 0:
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    ker_img = util.tensor2ker(ker_cur, min_max=(ker_cur.min(), ker_cur.max()))
                    save_ker_path = os.path.join(img_dir, 'ker_{:d}_org.png'.format(current_step))
                    util.save_img(ker_img, save_ker_path)
