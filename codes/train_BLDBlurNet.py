...
def BLDSRnet(opt_P, opt_C):
    ...
    # model.load_state_dict(torch.load('../checkpoints/usrnet.pth'), strict=False)
    #model.load_state_dict(torch.load('../experiments/blur_noise_sc1_Pre/models_usrfine/F_264000.pth'), strict=False)
    ...
    model.pk0     = DataParallel(model.pk0)
    
    #model.load_state_dict(torch.load('../experiments/train_USRit_bld_ker_USRfine/models_Pinit/F_36000.pth'), strict=True)
    #model.load_state_dict(torch.load('../experiments/blur_noise_sc1_Pre/models_init_ker/F_26000.pth'), strict=True)
     
    #pretrained_dict = torch.load('../experiments/train_USRit_bld_ker_USRfine/models_PH/F_40000.pth')
    # pretrained_dict = torch.load('../experiments/blur_noise_sc1_Pre/models_usrfine/F_264000.pth')
    # pretrained_dict = {k[:]: v for (k, v) in pretrained_dict.items() if ((k[0:2]=='p.') or (k[0:2]=='h.'))}
    # model.load_state_dict(pretrained_dict, strict=False)    
    
    #model.load_state_dict(torch.load('../experiments/blur_noise_sc1_Pre/models_usrfine/F_264000.pth'), strict=False)
    model.load_state_dict(torch.load('../experiments/blur_sc1_Pre/models_final_wrong/F_88000.pth'), strict=False)
    ...
    model.load_state_dict(torch.load('../experiments/blur_sc1_ker2/models/F_18500.pth'), strict=True)
    #model.load_state_dict(torch.load('../experiments/blur_noise_sc1_Pre/models/F_138000.pth'), strict=True)
    
    # pretrained_dict = torch.load('../checkpoints/usrnet.pth')
    # pretrained_dict = {k[:2]+'module.'+k[2:]: v for (k, v) in pretrained_dict.items()}
    # model.load_state_dict(pretrained_dict, strict=False) 
    ...
    for key, v in model.named_parameters():
        ...
        #if (key[0:2]=='p.') | (key[0:2]=='h.'):
        #if (key[0:6]=='p_init') | (key[0:6]=='h_init'):
        #if (key[0:2]=='p.'):
        #if (key[7]=='p') | (key[7]=='h'):
        if (key[0:2]=='pk'):
        #if (key[0:4]=='netP'):
...
def main():
    ...
    # torch.backends.cudnn.deterministic = True(key[0:4]=='netC') | 
    ...
    blur_loss = util.BatchBlur_wrap_multiker(l=21)
    ...
    noise_level = 0.03
    ba_size = opt_P['datasets']['train']['batch_size']
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            SR_img, ker_img, b_check, est_ker_map = model_SR(LR_img, b_kernels, opt_P['scale'], sigma, train_data['GT'])             
            ...
            # cropped_gt_img = train_data['GT'][:,:,crop_size:-crop_size, crop_size:-crop_size]
            # fl1=F1_loss(cropped_sr_img,cropped_gt_img.to(device))
            #fl=F1_loss(ker_est,b_kernels.view(B,1,H,W).to(device))
            #fl2=F2_loss(ker_img,b_kernels.view(B,1,H,W).to(device))
            #fl2=F2_loss(est_ker_map,kernel_code.to(device))
            reblur = blur_loss(train_data['GT'].to(device), ker_img.view(B,H,W))    
            cropped_lr_img = LR_img[:,:,crop_size:-crop_size, crop_size:-crop_size]
            cropped_rb_img = reblur[:,:,crop_size:-crop_size, crop_size:-crop_size]
            fl2 = F2_loss(cropped_rb_img,cropped_lr_img)
