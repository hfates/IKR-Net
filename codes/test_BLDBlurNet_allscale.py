...
def BLDSRnet(opt_P, opt_C):
    ...
    model = net(opt_P,opt_C,n_iter=8, h_nc=64, in_nc=1, out_nc=1, nc=[2, 4, 8, 16],
                       nb=2, act_mode="R", downsample_mode='strideconv', upsample_mode="convtranspose", mode="test")
    ...
    model.load_state_dict(torch.load('../experiments/blur_allscale/models/F_62000.pth'), strict=False)
    ...
    #model.load_state_dict(torch.load('../experiments/blur_sc1_ker2/models/F_34750.pth'), strict=True)
...
def main():
    ...
    crop_size = max(opt_P['scale_univ']**2, 10)
        ...
        #for filt_ind in [0]:
            ...
            if (1):
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    prepro_val = util.USRNetTest(opt_P['scale_univ'], pca_matrix, random=True  , para_input=opt_P['code_length'],
                                                              kernel=opt_P['kernel_size'], noise=False, cuda=True, sig=opt_P['sig'],
                    ...
                    LR_img = nn.functional.interpolate(LR_img, scale_factor=opt_P['scale_univ'], mode='bicubic')
                    ...
                    img_dir = opt_P['path']['val_images']+"/"+str(filt_ind+1)
                    ...
                    #sr_img = srnew[1:-1:2,1:-1:2,:]
                    shift_no = 1*(opt_P['scale_univ']==4)
