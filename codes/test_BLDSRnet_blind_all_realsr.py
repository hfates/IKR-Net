...
def main():
    ...
    parser.add_argument('-opt_P', type=str, default="options/test/test_Pred_EncDec_ex.yml", help='Path to option YMAL file of Predictor.')
        ...
        for filt_ind in [1]:
            ...
            if (1):
                ...
                for _, val_data in enumerate(val_loader):
                    ...
                    img_dir = opt_P['path']['val_images']+"/realsr_scale"+str(sc_final)+"/"+str(current_step)
                    ...
                    sigma = torch.tensor([0*noise_level]*B).float().view([B, 1, 1, 1]).to(device)
                    SR_it = []
                    ker_it = []
                    ...
                    with torch.no_grad():
                        SR_img, ker_est, _ = model_SR(val_data['GT'].to(device), b_kernels, opt_P['scale'], sigma, val_data['GT'], SR_it, ker_it) 
